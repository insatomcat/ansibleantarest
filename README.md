...
- EL 10 and CentOS Stream run the `antares_web` machine, not a Slurm node. OpenHPC 3 publishes an EL_9 tree and nothing above it, so EL 10 has nothing to install Slurm from. CentOS Stream provides `redhat-release 9.0` and always will, being what the next RHEL minor is built from rather than a rebuild of one, while the OpenHPC release package requires 9.1 or later. The `slurm_common` role stops with the reason in both cases. A cluster of EL 9 nodes driven by an EL 10 or CentOS Stream web server is a supported shape; a cluster of either is not.
