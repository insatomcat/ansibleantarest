...
- **Two copies of the solvers.** One under `antarest_base_dir` for the local launcher, one in the shared `/home` for the Slurm one, plus about 250 MB per Xpansion release. And the zipped results of every run land in `/home/antares`, and the study being run in `/var/tmp/antares` (see [scratch space](#scratch-space-and-the-final-zip)), and [putting the state on its own volume](antares-web.md#putting-the-state-on-its-own-volume) covers neither: that section is about `/var/antares-web`.
...
## Scratch space and the final zip

```yaml
slurm_scratch_dir: /var/tmp/antares   # on each compute node, created by slurm_compute
slurm_compute_packages: [7zip]        # what the launch script runs beyond the solvers
```

A job copies the zipped study into `slurm_scratch_dir/antares_<job id>`, unpacks it, runs it there, and writes the final zip straight into the shared `/home`, where antares-launcher downloads it from. The scratch directory is on the node's own disk because a study and its output are thousands of small files, and on NFS each of them costs round trips to the front-end, three times per run: when it is unpacked, when the solver writes it, and when it is zipped. The one file that has to reach `/home` gets there as one sequential stream. `/var/tmp` rather than `/tmp` because Debian 13 mounts `/tmp` as a tmpfs sized on the RAM, which is exactly what the solver needs for itself.

That disk is the root filesystem of the node, and a run needs the unpacked study and its whole output there at once. On nodes that run large studies, mount a local volume and point `slurm_scratch_dir` at it; `slurm_compute` creates the directory with the right owner wherever it is. A run that Slurm kills (time limit, `scancel`) leaves its directory behind, and under `/var/tmp` systemd-tmpfiles removes it after a month. Moving an existing cluster to this layout leaves an empty `/home/antares/scratch/` that nothing uses any more.

The final zip is built by 7-Zip, with `-tzip -mx=1` on as many threads as the job was given (`--cpus-per-task`, the number of cores chosen in the interface), where `zip -2` deflated on one. The format stays zip because both ends need it: antares-launcher looks for `finished_<study>_<job id>.zip` and Antares-Web opens it with Python's `zipfile`. The `7zip` package installs `7z` on every supported distribution, from EPEL on EL, and the script falls back to `zip -2` on a node where it is missing. `_job_data_<id>.txt` names the tool used, and `stats-antares-v1.1.txt` in `/home/antares` keeps `UNZIP_DURATION` and `ZIP_DURATION` for every job, which is where to look before and after.

The study is still unpacked with `unzip`: inflating is cheap on one core, and what made that step slow was writing its files over NFS. The zipping and unzipping done on the web machine, by antares-launcher and by Antares-Web, is Python code inside those two projects and is out of the playbook's reach.
