...
- **The monitoring**, when `monitoring_enabled` is on. Prometheus is asked, on the machine it is bound to, what its targets are doing: every machine of the inventory has to be among them - one that is missing is one whose facts were not in the play that rendered `prometheus.yml`, and nothing else reports that - and every one of them has to be answering. The Slurm exporter gets a question of its own, since it answers whether or not it reached the controller: `slurm_nodes_total` has to exist, which it only does once `sinfo` came back. Grafana is asked for its health through the front door, from the controller, which says at once that the container is up, that the route is there and that Grafana serves itself under the prefix it is given.
...
Every deployment gets the login checks, gated or not. With a password list in front of the interface or the consoles (see [The password in front of the password](edge-and-tls.md#the-password-in-front-of-the-password)), give `verify.yml` an account of each tier and its probes keep coming from the controller, through the door, the way a browser would:

```bash
ansible-playbook verify.yml \
    -e antares_edge_basic_auth_probe_user=alice \
    -e antares_edge_basic_auth_probe_password='...' \
    -e antares_edge_basic_auth_admin_probe_user=ops \
    -e antares_edge_basic_auth_admin_probe_password='...'
```

On the command line rather than in the inventory, since these are passwords in the clear and the lists the deployment is configured from hold hashes. Without them, a probe that has to get past a gated door is asked from the machine itself instead, whose loopback the door never challenges. Everything above still gets checked that way except the door's own behaviour: **from a trusted address the gate is off entirely**, so the exemptions it makes for everybody else are not exercised at all, and a missing one goes unnoticed. That is not theoretical, it is how the Keycloak console shipped broken on this branch.

Either way, the door is checked from the controller for what it is: a `401` carrying a `Basic` challenge on the interface, on the Keycloak console and on Grafana. And two checks ask what it forwards, by carrying a basic credential the way a browser does once it has been let through: the answer has to be the service's own rather than a complaint about the `Authorization` header. Put the controller's address in `antares_edge_basic_auth_trusted` to have every probe come from outside again.
