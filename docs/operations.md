...
With a password list in front of the interface or the consoles (see [The password in front of the password](edge-and-tls.md#the-password-in-front-of-the-password)), the controller has no password to give and no business having one: it is not in `antares_edge_basic_auth_trusted`. Each probe that has to reach what is behind the door is then asked from the machine itself, whose loopback that list does contain, and it proves what it always proved except the network path - which every other check in the file still crosses. The door is checked from the controller for what it is: a `401` carrying a `Basic` challenge, on the interface and on the Keycloak console and on Grafana, which is the check that the gate is really shut. Two more checks ask what the door forwards: a login and a Grafana health request are made carrying a basic credential of their own, the way a browser does once it has been let through, and the answer has to be the service's own rather than a complaint about the `Authorization` header. The three interfaces are logged into through the door in CI, which is where the passwords to do it live. Put the controller's address in `antares_edge_basic_auth_trusted` to have every probe come from outside again.
