...
The beat container keeps its "last run" state in a shelve file whose default name is relative to the working directory, which in this image is `/`, not writable by a container running as `antares_uid`. Its unit sets `CELERYBEAT_SCHEDULE=/celerybeat/celerybeat-schedule`, which the image's `start.sh` hands to `celery beat --schedule`, and `/celerybeat` is a bind mount of `data/celerybeat`, so the state also survives a restart. `start.sh` reads that variable from AntaREST 2.35 on, which is why the role refuses an older release (see [Antares-Web](antares-web.md#configuration)).
