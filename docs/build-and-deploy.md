...
In `archive` mode the target loads the images of a set it has not loaded yet, and only those: it keeps a copy of the last loaded `manifest.yml` in `loaded-manifest.yml` under `antarest_base_dir`, and a shipped manifest that differs from it (every `build.yml` stamps its own date) reloads the images and replaces the frontend. The Antares-Web image is always tagged `latest`, so its mere presence cannot tell a 2.34 from a 2.35. The target also unpacks the frontend in the checked-out tree where nginx bind-mounts it, and takes solvers from the local cache instead of GitHub. Archives are transported with `rsync`, not the `copy` module, which is unsuitable for hundreds of megabytes.
